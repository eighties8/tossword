      ...
      <header className="bg-white text-gray-900 py-4 px-6 border-b border-gray-300 shadow-sm">
        ...
        <div className="flex-1 flex flex-col items-center md:pt-4">
          ...
          {(gameState.gameWon ? [gameState.rootWord, ...gameState.attempts] : gameState.attempts.slice(-1)).map((attempt, sliceIndex) => {
            ...
            const entryOrder = gameState.gameWon ? sliceIndex : remainingSteps

            return (
              <div
                key={actualIndex}
                className={[
                  "mx-auto mb-2 w-full", // <-- add max width + side padding
                  "flex items-center justify-center gap-2",
                  gameState.gameWon &&
                  actualIndex < lastAttemptRowIndex &&
                  lastAttemptRowIndex - actualIndex <= gameState.winRevealRowsShown
                    ? "animate-[rowReveal_0.35s_ease-in_forwards]"
                    : "",
                ].join(" ")}
                style={{
                  display:
                    gameState.gameWon && actualIndex < lastAttemptRowIndex
                      ? lastAttemptRowIndex - actualIndex <= gameState.winRevealRowsShown
                        ? "flex"
                        : "none"
                      : "flex",
          ...
          {!gameState.gameWon && (
            <>
              <div className={`mx-auto overflow-hidden transition-[max-height,opacity,margin] duration-1300 ease-in-out ${gameState.showAutoHint ? 'max-h-12 my-2 opacity-100' : 'max-h-0 my-0 opacity-0'}`}>
                  ...
                  <div key={rIdx} className="flex justify-center gap-1 mb-1">
                    {row.map((k) => {
                      ...
                      return (
                        ...
                        >
                          {k === 'BACKSPACE' ? <div className="flex items-center justify-center w-full h-full"><Delete className="w-6 h-6" /></div> : k}
                        </button>
                      )
                    })}
                  </div>
